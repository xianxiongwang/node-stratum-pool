...
var varDiff = module.exports = function varDiff(port, varDiffOptions) {
    ...
    this.manageClient = function (client) {
        ...
        client.on('submit', function () {
            ...
            if(!( (ts - lastRtc) >= options.retargetTime || timeBuffer.getFull() || sinceLast>tMax )){
                return;
            }

            console.error("Difficulty update:ts - lastRtc="+(ts - lastRtc)+" getFull="+timeBuffer.getFull()+" sinceLast>tMax="+(sinceLast>tMax +" timeBuffer.size="+timeBuffer.size()));

            var avg = timeBuffer.avg(); 

            //上次时间正常
            if(sinceLast<=tMax && (avg>=tMin && avg<=tMax)){
                if((ts - lastRtc) >= options.retargetTime || timeBuffer.getFull()){
                    lastRtc = ts;
                    timeBuffer.clear();
                }
            ...
            var targetDiff = diffAvg/avg * options.targetTime;
            //var ddiff = options.targetTime / avg;

            //change diff
            console.error("Difficulty update:avg="+avg+" diffAvg="+diffAvg+" targetDiff="+targetDiff +" targetTime="+options.targetTime+" tMin="+tMin+" tMax"+tMax);
