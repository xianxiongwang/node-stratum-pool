...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function SubmitBlock(blockHex, callback) {
        emitLog('Submitted Block : ' + blockHex);
        ...
        function SubmitBlockHandleResult(results,callagain){
            var success = false;
            var rejectNum = 0;
            ...
            if(success){
                ...
                //all rejected
                if(rejectNum == results.length){
                    emitErrorLog('Daemon instance all node rejected:'+rejectNum);
                    return;
                }
                if(callagain){
                    emitLog('submit block error, submit again');
                    //call again
                    _this.daemon.cmd(rpcCommand,
                        rpcArgs,
                        function (resultstwo) {
                            SubmitBlockHandleResult(resultstwo,false);
                        }
                    );
        ...
        _this.daemon.cmd(rpcCommand,
            rpcArgs,
            function (results) {
                SubmitBlockHandleResult(results,true);
            }
        );
    }
